...
//        Map<Variable<Type>, Integer> collect = assignment.unassignedVariables().stream().collect(Collectors.toMap(Function.identity(), x -> 0));
//        constraints.stream()
//                .map(constraint -> assignment.subsetOf(constraint.reliesOn()))
//                .flatMap(Collection::stream)
//                .filter(collect::containsKey)
//                .peek(variable -> collect.put(variable, collect.get(variable) + 1))
